...
static int kvm_booke206_tlb_init(PowerPCCPU *cpu)
{
    ...
    ret = kvm_enable_cap_vcpu(cs, KVM_CAP_SW_TLB, (uintptr_t)&cfg);
    if (ret < 0) {
...
int kvmppc_booke_watchdog_enable(PowerPCCPU *cpu)
{
    ...
    ret = kvm_enable_cap_vcpu(cs, KVM_CAP_PPC_BOOKE_WATCHDOG);
    if (ret < 0) {
...
void kvmppc_set_papr(PowerPCCPU *cpu)
{
    ...
    ret = kvm_enable_cap_vcpu(cs, KVM_CAP_PPC_PAPR);
    if (ret) {
...
void kvmppc_set_mpic_proxy(PowerPCCPU *cpu, int mpic_proxy)
{
    ...
    ret = kvm_enable_cap_vcpu(cs, KVM_CAP_PPC_EPR, mpic_proxy);
    if (ret && mpic_proxy) {
